...
export default function Police() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const data = await getPoliceDrills();
        ...
        console.error(err);
        setError("Failed to load police drills.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const buildGeneratedPayload = (drill) => {
    ...
    return {
      ...
      vehicleInvolved: drill.vehicleInvolved || null,
      backupRequired: drill.backupRequired || null,
      legalContext: drill.legalContext || null
    };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingDrill) {
        ...
        const updated = await updatePoliceDrill(editingDrill.id, payload);
        ...
        const payload = buildManualPayload();
        const created = await createPoliceDrill(payload);
      ...
      console.error(err);
      setError("Failed to save police drill.");
    }
  };
  ...
  const handleDelete = async (category, drill) => {
    ...
    try {
      await deletePoliceDrill(drill.id);
      ...
      console.error(err);
      setError("Failed to delete police drill.");
    }
  };
  ...
  const generateDrill = async () => {
    try {
      ...
      const data = generatePoliceDrill(genData);
      setGenerated(data);
    } catch (err) {
      console.error(err);
      setError("Failed to generate police drill.");
    } finally {
      setGenLoading(false);
    }
  };
  ...
  const saveGenerated = async (drill) => {
    try {
      ...
      console.error(err);
      setError("Failed to save generated police drill.");
    }
  };
        ...
        {mode === "generator" && (
          ...
          >
            ...
            <div style={{ display: "grid", gap: 12, marginTop: 12 }}>
              ...
              <input
                placeholder="Focus (Patrol Skills, Traffic Stops, etc.)"
                value={genData.focus}
                onChange={(e) =>
              ...
              <input
                placeholder="Environment (Urban, Suburban, Rural, etc.)"
              ...
              <input
                placeholder="Objective (Contact, Detain, Search, etc.)"
              ...
              <input
                placeholder="Scenario Type (Traffic Stop, Domestic, etc.)"
              ...
              <input
                placeholder="Call Type (Priority, Routine, etc.)"
                value={genData.callType}
                onChange={(e) =>
                  setGenData({ ...genData, callType: e.target.value })
                }
                style={inputStyle}
              />
              ...
              <input
                placeholder="Suspect Profile (Armed, Unarmed, Fleeing, etc.)"
                value={genData.suspectProfile}
                onChange={(e) =>
                  setGenData({ ...genData, suspectProfile: e.target.value })
                }
                style={inputStyle}
              />

              <input
                placeholder="Vehicle Involved (Yes/No, Type)"
                value={genData.vehicleInvolved}
                onChange={(e) =>
                  setGenData({ ...genData, vehicleInvolved: e.target.value })
                }
                style={inputStyle}
              />

              <input
                placeholder="Backup Required (Yes/No, Units)"
                value={genData.backupRequired}
                onChange={(e) =>
                  setGenData({ ...genData, backupRequired: e.target.value })
                }
                style={inputStyle}
              />

              <input
                placeholder="Legal Context (Warrant, Consent, Exigent, etc.)"
                value={genData.legalContext}
                onChange={(e) =>
                  setGenData({ ...genData, legalContext: e.target.value })
                }
                style={inputStyle}
              />
            ...
            {generated && (
              ...
              >
                ...
                <p>
                  <strong>Threat Level:</strong> {generated.threatLevel}
                </p>
